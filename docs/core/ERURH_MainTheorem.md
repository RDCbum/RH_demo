...
growth of Γ and ζ′/ζ, dyadic zero counts) together with a finite set of ERURH
numeric certificates (alpha and beta), the ERURH system shows there are no
zeros of ζ with real part greater than 1/2. This yields a conditional proof
...
- The analytic inputs A1 and A2 are supplied by paper lemmas
  ...
  companion preprint.
- The theorem `RH_from_ERURH_conditional` lives in Lean and consumes an
  `ERURH_GlobalAssumptions` bundle that aggregates external hypotheses and
  window assumptions.
