...
Goal (Lean): `ERURH.Alpha.mode_to_rms_mode` in `lean/formal_proofs/ERURH/Alpha/ModeToRMSMode.lean`.
...
- Statement: `mode_to_rms_mode` (see `docs/core/RB34_BLOCKERS.md`).
...
- Paper anchor: Section \ref{sec:no-supercritical}, Lemma B.
  Sublemmas (from RB34 blockers):
  - B.1 pointwise growth \(\Rightarrow\) RMS lower bound for a fixed window.
...
Conclusion
- Combine Lemma B + Lemma C to show `¬ ERU_mode_beta β` for all `β > 1/2` and derive
  `ERURH.no_supercritical_beta`.
