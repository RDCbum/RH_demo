...
**Response.** A1 is a paper lemma with statement exported from Lean.  
**Evidence.**
- `arxiv_submission/ERURH_Conditional_Proof.tex` (Lemma `a1-from-supercritical`).  
- `lean/formal_proofs/ERURH/A1FromSupercriticalMode_Legacy.lean` (statement).  
...
**Response.** In the legacy window-free route, the context is abstract and the
RMS semantics live in the paper; Lean only uses the abstract interface.  
**Evidence.**
...
**Response.** The checklist separates the primary route (legacy window-free)
from the fixed-window route and marks the latter’s open gap explicitly.  
**Evidence.**
