...
`ctx`. -/
structure PlanB_AnalyticAssumptions (ctx : RMSLocalContext) where
...
def PlanB_AnalyticAssumptions.coeff_decay_Hb
  {ctx : RMSLocalContext}
...
def PlanB_AnalyticAssumptions.ls_gamma_fine
  {ctx : RMSLocalContext}
  (H : PlanB_AnalyticAssumptions ctx) :
  Prop :=
...
def PlanB_AnalyticAssumptions.classical_assumptions
  {ctx : RMSLocalContext}
  (H : PlanB_AnalyticAssumptions ctx) :
...
def PlanB_AnalyticAssumptions.of_simple_LS
  (ctx : RMSLocalContext)
...
by
  refine
    { spectral := spec
      ...
      A2_low_RMS := hLow
      A2_tail_RMS := ?_ }
  exact A2Tail_RMS_from_Hb_LS_simple ctx (SpectralAssumptionsAlpha.to_H_b spec)

/-- Convenience constructor from a bare proof of `H_b`. -/
def PlanB_AnalyticAssumptions.of_simple_LS_Hb
  (ctx : RMSLocalContext)
  (hExp : ExplicitFormulaERU)
  (hHb  : H_b)
