...
/-- Testigo explícito básico para el rectángulo analítico de `xiAlpha`.
    No introduce axiomas: `ExpRect` es un contenedor estructural. -/
def explicit_rect_alpha_true : explicit_rect_alpha :=
  ⟨{ log_bound := fun _ => (0 : ℝ)
     arg_bound := fun _ => (0 : ℝ)
     Tcap := (0 : ℝ) }⟩
...
theorem prefactor_bound_alpha_true (data : AlphaInterfaces) : prefactor_bound_alpha := by
  rcases data with ⟨analytic, cover, prefactor, lambda, zeta,
...
theorem xi_modulus_bound_alpha_true (data : AlphaInterfaces) : xi_modulus_bound_alpha :=
  ⟨alphaXiWitness data⟩

/-- Zeta bounds are witnessed directly by the axiomatic alpha interface bundle. -/
theorem zeta_modulus_bound_alpha_true (data : AlphaInterfaces) : zeta_modulus_bound_alpha := by
  rcases data with ⟨analytic, cover, prefactor, lambda, zeta,
    log_total_main, log_total_tail, log_to_bridge_main, log_to_bridge_tail,
    arg_total, arg_to_bridge, explicit, rh_from_E⟩
  refine ⟨cover, ?_⟩
...
theorem xi_arg_total_alpha_true (data : AlphaInterfaces) : xi_arg_total_alpha :=
  ⟨data, trivial⟩

/-- Testigo concreto para la hipótesis de proyección de argumento al bridge en alpha. -/
theorem xi_arg_to_bridge_alpha_true (data : AlphaInterfaces) : xi_arg_to_bridge_alpha :=
  ⟨data, trivial⟩

/-- Conjunción de las hipótesis concretas de argumento en alpha. -/
theorem xi_argument_hypotheses_alpha_instantiated_true (data : AlphaInterfaces) :
  xi_argument_hypotheses_alpha_instantiated :=
by
  refine And.intro ?h_total ?h_bridge
  · exact xi_arg_total_alpha_true data
  · exact xi_arg_to_bridge_alpha_true data

/-- Sub-axioma global de argumento derivado de las hipótesis concretas. -/
theorem xi_argument_alpha_true (data : AlphaInterfaces) : xi_argument_alpha :=
by
  apply xi_argument_alpha_of_instantiated
  exact xi_argument_hypotheses_alpha_instantiated_true data

/-- Testigo concreto para la representación integral explícita en alpha. -/
theorem explicit_integral_representation_alpha_true (data : AlphaInterfaces) :
  explicit_integral_representation_alpha :=
by
  exact ⟨alphaExplicitWitness_from_inertia data inertia_alphaBridge⟩

/-- Conjunción de hipótesis core para la fórmula explícita en alpha. -/
theorem explicit_core_hypotheses_alpha_instantiated_true (data : AlphaInterfaces) :
  explicit_core_hypotheses_alpha_instantiated :=
by
  refine And.intro ?hRect ?hRest
  · exact explicit_rect_alpha_true
  · refine And.intro ?hIntegral ?hSplit
    · exact explicit_integral_representation_alpha_true data
    · trivial

/-- Sub-axioma global `explicit_core_alpha` derivado de las hipótesis concretas. -/
theorem explicit_core_alpha_true (data : AlphaInterfaces) : explicit_core_alpha :=
by
  apply explicit_core_alpha_of_instantiated
  exact explicit_core_hypotheses_alpha_instantiated_true data
