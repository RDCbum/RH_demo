...
theorem RH_unconditional_try : RiemannHypothesis xiAlpha := by
  refine RH_from_ERURH_conditional ?G ?hAxioms certificatesCorrectAlpha_true ?hNumeric
  · -- Build the global assumptions bundle to surface required fields.
    refine ERURH_GlobalAssumptions.mk ?eru ?window
    · refine ERURH_Assumptions.mk ?classical ?spectral ?ls_gamma ?explicit_formula_ERU
        betaInertiaAssumptions_true
